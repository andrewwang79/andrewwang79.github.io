...
```
1. [重置branch](https://blog.csdn.net/weixin_33974433/article/details/87963137)，上面的删除提交方法无效了
```
gitclone后进入目录，用空分支替换master分支
git checkout --orphan new_branch && rm * -rf
echo linux > README.md && git add -A && git commit -am "Initial commit" // 没有文件无法创建commit，就没法替换master
