...
1. 切换分支/标签
  ...
  1. 下载远程分支并切换：git checkout -b <本地分支> origin/<远程分支>，git checkout -b lb origin/rb
  1. 下载远程标签并切换：git checkout -b <本地分支> origin/tag1
1. 基于当前分支创建新分支/切换分支：git branch <分支>
1. 删除分支：git branch -d <远程分支>

### commit
1. 添加所有的文件，包括删除的：git add -A .
...
### 其他
1. git add . && git commit -m "msg" && git push // 快速提交
